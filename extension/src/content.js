(function (root) {
  ...
  if (adapter.ensureProfileImport) adapter.ensureProfileImport();
  // Scholar builds the profile table progressively. Retry after it has mounted so
  // the import action is available even when the content script runs early.
  if (adapter.ensureProfileImport) {
    setTimeout(() => adapter.ensureProfileImport(), 1200);
    setTimeout(() => adapter.ensureProfileImport(), 3500);
  }
  ...
  function hasLookupSeed(entry) {
    return entry && entry.anchorEl && (entry.issn || entry.journalName || entry.doi || entry.paperTitle);
  }

  function applyResolvedJournal(entry, resolved) {
    if (!entry || !resolved) return false;
    if (resolved.name && !entry.journalName) entry.journalName = resolved.name;
    if (resolved.issn && !entry.issn) entry.issn = resolved.issn;
    return !!(entry.journalName || entry.issn);
  }

  async function resolveEntriesBeforeLookup(rawEntries) {
    if (!adapter.resolveJournalName) return;
    const targets = rawEntries.filter((entry) => {
      const key = ns.lookup.queryKey(buildItem(entry));
      return !key && (entry.doi || entry.paperTitle);
    }).slice(0, 12);
    if (!targets.length) return;
    await Promise.all(targets.map(async (entry) => {
      try {
        applyResolvedJournal(entry, await adapter.resolveJournalName(entry));
      } catch (_) {}
    }));
  }
  ...
  async function retryMissesWithResolvedNames(unique, results) {
    ...
    for (let i = 0; i < unique.length; i += 1) {
      ...
      unique[i].item = item;
      (unique[i].entries || []).forEach((e) => {
        if (item.name) e.journalName = item.name;
        if (item.issn) e.issn = item.issn;
      });
    }
  ...
  async function scan() {
    ...
    try {
      ...
      const rawEntries = (adapter.findEntries() || [])
        .filter(hasLookupSeed);
      await resolveEntriesBeforeLookup(rawEntries);
