(function (root) {
  ...
  function canShowPublishFee(settings) {
    ...
    if (settings && settings.allowPublishFee === false) return false;
    return false;
  }
  ...
  function makeBadges(journal, settings) {
    const j = journal || {};
    const s = settings || {};
    const showFee = s.showFree !== false && canShowPublishFee(s);
    if (Array.isArray(j.display_badges) && j.display_badges.length) {
      const enabled = (badge) => {
        ...
        if (badge.group === 'access') return showFee;
    ...
    add(out, showFee, j.free ? '免费发表' : '', 'pill free');
    add(out, showFee, j.doaj_apc === 'no' ? 'DOAJ 免 APC' : '', 'pill free');
  ...
  function renderBadges(journal, settings) {
    ...
    if (journal && journal.slug) {
      const detailUrl = `https://journal.ailatest.org/#j/${encodeURIComponent(journal.slug)}`;
      ...
      link.addEventListener('click', (event) => {
        ...
        if (!opened) location.assign(detailUrl);
      });
