...
function renderJournalPage(journal, oaData, baseUrl) {
  ...
  const currentIf = journal.if_2025 ?? journal.if_2024;

  // Build description
  const descParts = [title];
  if (currentIf) descParts.push(`IF: ${currentIf}`);
...
<body>
  <div class="ssr-container">
    ...
    <!-- Stats Grid -->
    ${currentIf || zone2023 != null || reviewCycle ? `
    <div class="ssr-section">
      <h2>Journal Metrics</h2>
      <div class="ssr-note">JCR 2026 release · 2025 metric</div>
      <div class="ssr-grid">
        ${currentIf ? `
        <div class="ssr-stat">
          <div class="ssr-stat-v">${escape(String(currentIf))}</div>
          <div class="ssr-stat-k">Impact Factor (2025 metric)</div>
